#!/usr/bin/env python3
"""
Build Q1 2026 Performance Dashboard.
All call data is from Q1 2026, revenue is annual (March 2025-2026).
"""
...
def main():
    ...
    for rep_id, rep_data in zoho_data['cross_validation'].items():
        name = rep_data['name']
        
        # Get Kixie data
        kixie_data = rep_data.get('kixie_data', {})
        kixie_email = kixie_data.get('email', '').lower()
        
        # Skip if no Kixie email match
        ...
        # Get revenue from full intelligence report
        revenue = 0
        for intel_rep_id, intel_rep_data in intel_data['cross_validation'].items():
            if intel_rep_data['name'] == name:
                rev_data = intel_rep_data.get('revenue', {})
        ...
        # Determine rating based on revenue per call and efficiency
    ...
    print(f"\nQ1 2026 Dashboard: {len(dashboard_data)} reps with complete data")
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        tr:nth-child(-n+3) { background: #fef3c7; }
